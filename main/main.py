...
with open('brdc0970.24n','r') as file:
    # 跳过前8行读取后面数据
...
# 修改toc为周内秒形式
cor = [24*6*3600]*len(df["toc"])
df["toc"] = cor
# 输出为excel表格
df.to_excel('brdc0970.24n.xlsx',index=False)
print(df)
...
# 创建一个结果表格的表头
key2 = ["positionX", "positionY", "positionZ", "Vx", "Vy", "Vz", "sat_Clk_error"]
# 创建一个存储表格
